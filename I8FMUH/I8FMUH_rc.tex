...
\begin{figure}[h]
	\centering
	\begin{tikzpicture}
		\newcommand\COR{1}; %korrekciós érték az ábra késsöbbi méreteinek változtatásához
		...
		%turbina T betűje
		\node[thick] at (3.9*\COR, -2.5*\COR) {T};
		...
		%q_EL bejelölése
		\draw[->] (5.7*\COR, -4.5*\COR) -- (7*\COR, -4.5*\COR) node [midway, below] {$q_K$};
		%q_BE bejelölése
